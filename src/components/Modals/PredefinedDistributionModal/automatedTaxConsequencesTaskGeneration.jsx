    ...
    if (distributor.location === 'Switzerland' && recipient.location === 'Switzerland') {
        ...
        if (response) {
            ...
            const createTaskThree = async () => {
                const newTask = {
                    title: 'WHT tax notification',
                    description: `Preparation of Swiss WHT notification (Form 108) between ${distributor.name} and ${recipient.name} in case a valid treaty clearance is available.
                                  In case no valid treaty clearance is available, Form 25 needs to be filed.`,
                    planned_completion_date: convertDate(datePlus30Days),
                    due_date: convertDate(datePlus30Days),
                    step_id: step.id,
                    documents: [],
